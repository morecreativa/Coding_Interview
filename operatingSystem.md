...
4) 실제의 물리 메모리 개념과 사용자의 논리 메모리 개념을 분리한 것이다.-> 작은 메모리를 가지고도 얼마든지 큰 **가상 주소 공간**을 프로그래머에게 제공할 수 있다.
5) 가상 주소 공간이란? 한 프로세스가 메모리에 저장되는 논리적인 모습을 가상메모리에 구현한 공간이다. **프로세스가 요구하는 메모리 공간을 가상메모리에서 제공함으로서 현재 직접적으로 필요치 않은 메모리 공간은 실제 물리 메모리에 올리지 않은 것으로 물리 메모리를 절약할 수 있다.**
6) 한 프로그램이 실행되며 논리 메모리로 100kb가 요구되었다고 하자. 하지만 실행까지에 필요한 메모리 공간(**Heap영역, Stack영역, 코드, 데이터**)의 합이 40kb라면 실제 물리 메모리에는 40kb만 올라가 있고, 60kb만큼은 **필요시에만 물리메모리에 요구한다**고 이해할 수 있겠다.
7) **시스템 라이브러리**가 여러 프로세스들 사이에 공유할 수 있도록 해줌. 각 프로세스들은 **공유 라이브러리**를 자신의 가상 주소공간에 두고 사용하는 것처럼 인식을 한다. 하지만, 라이브러리가 올라가 있는 **물리 메모리 페이지**들은 모든 프로세스에 공유하고 있다.
8) 프로세스들이 메모리를 공유하는 것을 가능케하고, 프로세스들은 공유 메모리를 통해 서로 통신이 가능하다. 각 프로세스들은 각자 자신의 주소공간 처럼 인식을 하지만, 실제 물리 메모리는 공유 되고 있다. 
9) **fork**()를 통해 프로세스 생성 과정에서 페이지들이 공유되는 것을 가능하게 한다.
10) **Demand Paging**: 프로그램 실행 시작 시에 프로그램 전체를 디스크(Pool)에서 물리 메모리에 적재하는 대신, 초기에 필요한 것들만 적재하는 전략을 요구 페이징이라 한다. 
* 가상메모리는 대개 페이지로 관리됨.
* 요구 페이징을 실행 과정에서 필요해질 때 페이지들이 적재된다. 한번도 접근되지 않는 페이지는 물리 메모리에 적재되지 않는다.
* 사용되지 않을 페이지를 가져오는 시간낭비와 메모리 낭비를 줄일 수 있다.
11) 물리 메모리가 모두 사용중인 상황에서의 메모리 교체 흐름
* 디스크에서 필요한 페이지의 위치를 찾는다.
* 빈 페이지 프레임을 찾는다. 1) **페이지 교체 알고리즘**을 통해 희생될 페이지를 고른다. 2) 희생될 페이지를 디스크에 기록하고, 관련 페이지 테이블을 수정한다.
* 새롭게 비워진 페이지 테이블 내 프레임에 새로운 페이지를 읽어오고, 프레임 테이블을 수정한다.
* 사용자 프로세스를 재시작한다.
### 페이지 교체 알고리즘
1) FIFO: First in First out-> Belady의 모순-> 페이지를 저장할 수 있는 페이지 프레임의 개수를 늘려도 되려 페이지 부재가 더 많이 발생하는 모순이 존재한다.
2) OPR: Optimal Page Replacement-> 앞으로 가장 오랫동안 사용되지 않을 페이지를 찾아 교체하는 것이다. 모든 프로세스의 참조 계획을 알고 있어야하지만 그게 말이됨? 그래서 구현 어려움!
3) LRU: Least Recently used-> 최적 알고리즘의 근사 알고리즘으로, 가장 오랫동안 사용되지 않은 페이지를 선택하여 교체함.(FIFO보다는 우수하고, OPT알고리즘 보다는 그렇지 못함)
4) LFU: Least Frequently used-> 참조 횟수가 가장 적은 페이지를 교체하는 방법. 활발하게 사용되는 페이지는 참조 횟수가 많아질거라는 가정하에 만들어진 알고리즘. (잘 안쓰임)
5) MFU: Most Frequently Used-> 참조 횟수가 가장 작은 페이지가 최근에 올라왔고, 앞으로 계속 사용될 것이라는 가정에 기반한다.

</br>

## 11. 캐시
1) 캐시메모리: 속도가 빠른 장치와 느린 장치간의 속도차에 따른 병목현상을 줄이기 위한 범용 메모리. **프로세서 가까이에 위치하면서 빈번하게 사용되는 데이터를 저장하는 장소!!!!**
2) CPU가 어떤 데이터를 원할 것인가를 어느정도 예측할 수 있어야 한다.
3) 캐시의 성능 판별: 작은 용량의 캐시 메모리에 CPU가 이후에 참조할, 쓸모있는 정보가 어느 정도 들어왔냐에 따라 성능이 좌지우지됨.
4) 적중율(**Hit rate**)을 극대화 시키기 위해 데이터 지역성의 원리를 사용한다.
* Locality: 모든 코드나 데이터를 균등하게 Access하는 것이 아닌, 특정 순간에 특정 부분을 집중적으로 참조하는 특성을 나타낸 것이다.
* Time Locality: 시간 지역성은 최근에 참조된 주소의 내용은 조만간 다시 참조된다.
* Spatial Locality: 대부분의 실제 프로그램이 참조된 주소와 인접한 주소의 내용이 다시 참조되는 특성
### 캐싱라인
1) 캐시가 아무리 가까워도 참조하고자 하는 데이터가 어느 곳에 저장되어 있는지 몰라 모든 데이터를 순회해야 한다면 시간이 오래걸리게 된다!
2) 캐시에 목적 데이터가 저장되어 있다면 바로 접근하여 출력할 수 있어야 캐시가 의미가 있어진다.
3) 그래서 생긴것이 바로 **캐싱라인**임->캐시에 데이터를 저장할 때 특정 자료구조를 사용하여 묶음으로 저장한다.
4) 다양한 주소에 있는 데이터를 사용하기 때문에 빈번하게 사용하는 데이터 또한 아주 흩어져 있음.
5) 캐시에 저장하는 데이터에는 데이터의 메모리 주소 등을 기록해둔 태그를 달아놓을 필요가 있다.
* 이러한 태그들의 묶음을 캐싱라인이라고 하고, 캐싱 라인을 기준으로 데이터를 가져옴.
* Full Associative: 모든 캐시라인의 태그에 비교회로를 장착하여 프로세스에 요청하는 주소와 태그 간 일치 여부를 병렬로 체크함. 자유도는 높다. 태그를 기억하는 메모리보다 큰 크기를 요구함.
* Set Associative: 각각의 메모리 주소에 대응하는 캐시 라인을 여러개 갖추도록 한다. 이 캐시라인의 그룹을 '세트'라고 함. **LRU**가 상당히 작동이 잘됨. 캐시를 잘 사용하는 프로그래밍...
* Direct Map: 하나의 라인 주소마다 하나의 캐시 라인이 대응된다. **Thrashing**이 발생할 수 있음. 동일한 캐시 라인에 대응하는 서로 다른 메모리 주소 데이터를 서로 번갈아 가며 엑세스 하면 문제가 발생한다. **메모리 주소에 대응하는 캐시라인이 직접 정해져 있어서 캐시 라인의 경합이 쉽게 발생함!**
